...
<script>
    ...
    function initPagination() {
        ...
        var properties = {
            ...
            callback: pageSelectCallback,                       // 调用回调函数
            items_per_page:${requestScope.pageInfo.pageSize},   // 每页显示几条
                        ...
                        <table class="table  table-bordered">
                            ...
                            <tbody>
                                <c:if test="${(empty requestScope.pageInfo.list)}">
                                    <%--MyBatis的pageHelper插件已经帮我们做了这件事，
                                    如果不打开插件的超出页数默认查最后一页这个功能而使用这种方法的话，
                                    人为修改pageNum使数值过大，将会导致重定向次数过多而使浏览器异常
                                    --%>
                                    <%--<c:choose>
                                        <c:when test="${requestScope.pageInfo.pageNum eq 0}">
                                            <tr>
                                                <td colspan="6" align="center">抱歉！没有查询到您要的数据！</td>
                                            </tr>
                                        </c:when>
                                        <c:when test="${requestScope.pageInfo.pageNum ne 0}">
                                            <tr>
                                                <td colspan="6" align="center">抱歉！没有查询到您要的数据！</td>
                                            </tr>
                                            <c:redirect url="/admin/get/page.html?pageNum=${requestScope.pageInfo.pageNum-1}&keyword=${param.keyword}"></c:redirect>
                                        </c:when>
                                    </c:choose>--%>
                                    <tr>
                                        <td colspan="6" align="center">抱歉！没有查询到您要的数据！</td>
